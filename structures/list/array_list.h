...
namespace structures
{
	...
	inline void ArrayList<T>::enlarge()
	{
		...
		Array<T>::copy(*this->array_, 0, *biggerArray, 0, size_);
		delete array_;
		array_ = biggerArray;
	}
	...
	inline Iterator<T>& ArrayList<T>::ArrayListIterator::operator=(Iterator<T>& other)
	{
		if (this != &other) {
			...
			arrayList_ = otherIterator.arrayList_;
		}
		return *this;
	}
	...
	{
		ArrayListIterator* otherIterator = dynamic_cast<ArrayListIterator*>(&other);
		return position_ != otherIterator->position_ || arrayList_ != otherIterator->arrayList_ || otherIterator == nullptr;
	}
