...
namespace structures
{
	...
	inline T& BinarySearchTree<K, T>::find(const K& key)
	{
		...
		else throw std::logic_error("could not find key");
	}
	...
	inline typename BinarySearchTree<K, T>::BSTTreeNode* BinarySearchTree<K, T>::findBSTNode(K key, bool& found)
	{
		...
		if (current == nullptr) {
			...
			while (current->accessData()->getKey() != key) {
	...
	inline void BinarySearchTree<K, T>::extractNode(BSTTreeNode* node)
	{
		...
		if (node == this->binaryTree_->getRoot())
		{
		...
		else if (node->hasLeftSon() && !node->hasRightSon())
		{
			...
			if (node->isLeftSon())
			{
				parent->changeLeftSon(leftSon);
				node->changeLeftSon(nullptr);
			}
			else {
				parent->changeRightSon(leftSon);
		...
		else if (!node->hasLeftSon() && node->hasRightSon())
		{
			...
			if (node->isLeftSon())
			{
				parent->changeLeftSon(rightSon);
				node->changeLeftSon(nullptr);
			}
			else {
				parent->changeRightSon(rightSon);
				node->changeRightSon(nullptr);
			}
			node->resetParent();
		}
		...
		{
			BSTTreeNode* newNode = node->getRightSon();
