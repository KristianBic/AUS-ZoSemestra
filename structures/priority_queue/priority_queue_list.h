...
namespace structures
{
    ...
    {
        int indexPriority = indexOfPeek();
        if (indexPriority != -1)
        {
            PriorityQueueItem<T>* item = list_->removeAt(indexPriority);
    ...
    inline T& PriorityQueueList<T>::peek()
    {
        ...
        if (indexPriority != -1)
        {
            return this->list_->at(indexPriority)->accessData();
        }
        else {
            throw std::logic_error("Priority queue is empty!");
        }
    ...
    inline int PriorityQueueList<T>::peekPriority()
    {
        ...
        if (indexPriority != -1)
        {
            return this->list_->at(indexPriority)->getPriority();
        }
        else {
            throw std::logic_error("Priority queue is empty!");
        }
