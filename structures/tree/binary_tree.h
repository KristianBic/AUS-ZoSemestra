...
namespace structures
{
	...
	{
		return dynamic_cast<BinaryTreeNode<T>*>(KWayTreeNode<T, 2>::getSon(this->LEFT_SON));
	}

	template<typename T>
	inline BinaryTreeNode<T>* BinaryTreeNode<T>::getRightSon()
	{
		return dynamic_cast<BinaryTreeNode<T>*>(KWayTreeNode<T, 2>::getSon(this->RIGHT_SON));
	}

	template<typename T>
	inline void BinaryTreeNode<T>::setLeftSon(BinaryTreeNode<T>* leftSon)
	{
		delete KWayTreeNode<T, 2>::replaceSon(leftSon, this->LEFT_SON);
	}

	template<typename T>
	inline void BinaryTreeNode<T>::setRightSon(BinaryTreeNode<T>* rightSon)
	{
		delete KWayTreeNode<T, 2>::replaceSon(rightSon, this->RIGHT_SON);
	}

	template<typename T>
	inline BinaryTreeNode<T>* BinaryTreeNode<T>::changeLeftSon(BinaryTreeNode<T>* leftSon)
	{
		return dynamic_cast<BinaryTreeNode<T>*>(KWayTreeNode<T, 2>::replaceSon(leftSon, this->LEFT_SON));
	}

	template<typename T>
	inline BinaryTreeNode<T>* BinaryTreeNode<T>::changeRightSon(BinaryTreeNode<T>* rightSon)
	{
		return dynamic_cast<BinaryTreeNode<T>*>(KWayTreeNode<T, 2>::replaceSon(rightSon, this->RIGHT_SON));
	}

	template<typename T>
	inline bool BinaryTreeNode<T>::isLeftSon()
	{
		return TreeNode<T>::getBrother(this->LEFT_SON) == this;
	}

	template<typename T>
	inline bool BinaryTreeNode<T>::isRightSon()
	{
		return TreeNode<T>::getBrother(this->RIGHT_SON) == this;
	}
	...
	inline void BinaryTree<T>::InOrderTreeIterator::populatePath(TreeNode<T>* current)
	{
		if (current != nullptr)
		{
			BinaryTreeNode<T>*binaryCurrent = dynamic_cast<BinaryTreeNode<T>*>(current);

			populatePath(binaryCurrent->getLeftSon());
			TreeIterator::path_->push(current);
			populatePath(binaryCurrent->getRightSon());	
		}
	}
}
